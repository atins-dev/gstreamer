...
static guint d3d11_window_signals[SIGNAL_LAST] = {
    0,
};
...
void gst_d3d11_window_on_key_event(GstD3D11Window *window, const gchar *event,
                                   const gchar *key)
{
...
void gst_d3d11_window_on_mouse_event(GstD3D11Window *window, const gchar *event,
                         ...
                         DXGI_FORMAT display_format, GError **error)
{
  ...
  GST_DEBUG_OBJECT(window, "Prepare window, display resolution %dx%d, caps %" GST_PTR_FORMAT, display_width, display_height, caps);

  return klass->prepare(window, display_width, display_height, caps,
                        video_processor_available, display_format, error);
}
...
void gst_d3d11_window_set_title(GstD3D11Window *window, const gchar *title)
{
  GstD3D11WindowClass *klass;
